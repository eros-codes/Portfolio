/* RoofMap — written by my teammate for the Reservation front end.
   Copied unchanged from the project so the case study draws the
   floor plan with the real renderer. */
...
/* Default seat-layout logic removed — chair positions now come only from DB */
...
/* Sofa module removed — lounge visuals are no longer generated by code */
...
function createTableTop(table) {
	...
	group.append(
		isRound
			? svgEl('ellipse', { class: 'table-highlight', cx: -w * 0.08, cy: -h * 0.08, rx: w * 0.31, ry: h * 0.2 })
...
/* Lounge drawing removed — seating is provided from DB only */
...
export class RoofMap {
	...
	render() {
		...
		this.tables.forEach((table, index) => {
			...
			anchor.style.setProperty('--table-delay', this.hasRendered ? '0ms' : `${Math.min(index * 18, 420)}ms`);
			...
			// چیدمان صندلی فقط از دیتابیس می‌آید؛ هیچ پیش‌فرضی در کد یا فایل نقشه نیست
			if (Array.isArray(table.chairs)) {
			...
			spin.append(createTableTop(table));
	...
	bindTableEvents(anchor, table) {
		...
		const finish = (event) => {
			...
			// اگر click شلیک نشه، moved روی true می‌مونه و کلیک بعدی بلعیده می‌شه
			else moved = false;
		};
